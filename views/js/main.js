...
var resizePizzas = function(size) {
  ...
  changeSliderLabel(size);
   ...
   var windowWidth = document.getElementById("randomPizzas").offsetWidth;
	...
	//modify the determineDx function into size Switcher
  ...
  function changePizzaSizes(size) {
	  //extract DOM outside the loop to save
	var randomPizzaContainer = document.getElementsByClassName("randomPizzaContainer");
    for (var i = 0; i < randomPizzaContainer.length; i++) {
	  randomPizzaContainer[i].style.width = (sizeSwitcher(size) * windowWidth) + "px";
	}
  }
...
function updatePositions() {
  ...
  //move document body scrolltop outside the loop
  var pizzaPosition = document.body.scrollTop / 1250;
  //define phase outside the loop
  var phase;
  for (var i = 0; i < items.length; i++) {
    phase = Math.sin((pizzaPosition) + (i % 5));
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  //resize the pizza number 
  var pizzaSize = window.screen.height / 5 / cols;
